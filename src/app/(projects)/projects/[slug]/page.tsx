...
export default async function ProjectDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { prev, next } = getPrevNext('projects', post.slug)
  return (
    <div style={{ maxWidth: 860, margin: '24px auto', padding: '0 16px', display: 'grid', gap: 24 }}>
      ...
      <header>
        <p style={{ color: 'color-mix(in oklab, var(--foreground), transparent 35%)' }}>Project • <time dateTime={post.date}>{new Date(post.date).toDateString()}</time></p>
